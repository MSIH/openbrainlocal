...
- The registry is machine-readable at `GET /api/v1/ingest/types` so connectors can self-check at startup. Since `x-` types are unregistered by design, there's no static list for them — the same endpoint's `extension_types` field (`[{type, count, default_searchable: false}]`, `GLOB 'x-?*'`-derived so it stays case-consistent with `isExtensionType()` and requires the same "at least one character after the prefix") reports which ones actually exist in the store, derived from `artifacts` at read time (#244). The MCP `list_types` tool surfaces the same registry + extension_types for an MCP-only caller.
