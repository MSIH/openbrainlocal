...
4. Repo: `lifecontext-connector-devsession` (own repo — distribution model starts on day one)
...
7. Repo: `lifecontext-connector-imessage`
...
5. Repo: `lifecontext-connector-photo-exif` (worker lives with core or alongside — decide here, it's core enrichment per doc 04's transducer split)
...
5. `awesome-lifecontext-connectors` list seeded with the three reference connectors + the template
...
- Every connector gets its own repo from day one; core repo stays contract + core
