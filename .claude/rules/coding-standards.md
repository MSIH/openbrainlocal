...
## Required
- **CI must be green before merge.** `.github/workflows/ci.yml` runs `npm test` + `npm run check:boundary` on Ubuntu/Windows × Node 20/22 for every PR — the server-side backstop to the local `.claude/hooks` gates. A red run blocks merge; don't merge around it.
