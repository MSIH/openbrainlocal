#!/usr/bin/env bash
# Acceptance tests for issue #13 — cloud-issue-gate.sh + draft-issue-gate.sh method-awareness.
set -u
...
# --- cloud-issue-gate ---
out=$(run_gate true "$FAKE" "$FAKE/src/db.js"); check "cloud, repo .js, no marker -> deny" deny "$out" $?
...
out=$(run_gate false "$FAKE" "$FAKE/src/db.js"); check "non-cloud -> allow" allow "$out" $?

echo junk > "$FAKE/.claude/.cloud-issue-done"
out=$(run_gate true "$FAKE" "$FAKE/src/db.js"); check "cloud, junk marker -> deny" deny "$out" $?
echo 13 > "$FAKE/.claude/.cloud-issue-done"
out=$(run_gate true "$FAKE" "$FAKE/src/db.js"); check "cloud, valid marker -> allow" allow "$out" $?
