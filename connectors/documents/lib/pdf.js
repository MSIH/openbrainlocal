...
export function needsOcr(text, numpages) {
  ...
  return meaningful / numpages < OCR_CHARS_PER_PAGE;
}

// PDF dates look like D:20190304143000+05'30' (offset quoted per the spec), with every
// component after the year optional and the D: prefix itself optional in the wild.
...
export async function extractPdf(buffer) {
  // verbosity 0 = errors only; without it pdfjs warns per file about the fake worker and
  // missing standard-font data, neither of which affects text extraction.
  ...
  try {
    ...
    if (info?.Title) meta.title = String(info.Title);
