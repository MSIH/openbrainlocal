> **This is a copy.** Source of truth: [`docs/04-connector-contract.md`](https://github.com/msih/life-context/blob/2.0/docs/04-connector-contract.md) in `msih/life-context`. Kept here so connector authors don't need a second checkout to read it; it can lag the source after a core-side edit — check there if something here looks stale. Mirrored as of `msih/life-context` commit `bb9252f` (PR #30).
...
> design-only ([`05-roadmap.md`](https://github.com/msih/life-context/blob/2.0/docs/05-roadmap.md) Milestone 0+). The contract is declared
> v1-stable only after three real connectors have used it (Milestone 5). This doc supersedes
> the ingestion-pipeline framing in [`03-ob2-design.md`](https://github.com/msih/life-context/blob/2.0/docs/03-ob2-design.md) §3 — see the naming
...
- **Live.** The payload schema is published as JSON Schema in the core repo
  ([`schemas/ingest.v1.json`](https://github.com/msih/life-context/blob/2.0/schemas/ingest.v1.json)) — connectors can validate in CI
