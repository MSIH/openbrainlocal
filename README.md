...
- **MCP** (Streamable HTTP) — `/mcp`, tools:
  - `store_memory` / `search_memories` — the original note store + recall (unchanged on the wire); `store_memory` also takes an optional `type` (a registered type or an `x-` extension marker, default `note`) to write dev/personal-isolating memories directly through MCP
