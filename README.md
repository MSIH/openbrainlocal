...
- **MCP** (Streamable HTTP) — `/mcp`, tools:
  - `store_memory` / `search_memories` — the original note store + recall (unchanged on the wire); `store_memory` also takes an optional `type`, `"note"` (default) or an `x-` extension marker (e.g. `"x-dev-note"`), to write dev/personal-isolating memories directly through MCP. Other registered types are rejected — `store_memory` only ever writes a freeform manual note (no `source_id`/`extra`/`occurred_at`), so a caller needing a real `photo`/`contact`/`digest` row (or an upsert key) still goes through `POST /api/v1/ingest`
