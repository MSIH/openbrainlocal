...
Real imports are messy. Curate contacts — fix emails/phones/addresses, set a photo, and wire up relationships (spouse/parent/child/`worksAt`) — in the browser at **`http://localhost:3000/ui/contacts.html`** (enter your `LIFECONTEXT_API_KEY` once). See [`docs/09-contacts-ui.md`](docs/09-contacts-ui.md). Exposing the UI beyond localhost (e.g. a Cloudflare Tunnel)? Set `UI_URL_TOKEN` in `.env` to gate it behind a bookmarkable capability URL — the UI then serves only at `/<token>/ui/chat.html` and `/<token>/ui/contacts.html` (token-first, matching the MCP `/<token>/mcp` URL; bare `/ui` 404s) and the bookmark authorizes with no manual key entry; see [`docs/07`](docs/07-cloudflare-tunnel-setup.md#opening-the-browser-ui-remotely-capability-url) for the URL-leak tradeoff and the Cloudflare Access recommendation.
