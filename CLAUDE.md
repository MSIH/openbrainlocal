...
connectors/              — external HTTP connectors, one self-contained folder each (doc 04; NEVER import src/ — `npm run check:boundary`). `photo-exif` is the single photo/video connector (EXIF + Google Takeout + folder-name person hints; `gphotos-takeout` was folded into it, #171)
...
| MCP (Streamable HTTP) | `/mcp` — tools `store_memory`, `search_memories`, `search`, `timeline`, `about_entity`, `get_artifact` + entity curation (`merge_entities`, `propose_entity`, `list_proposed_entities`, `approve_proposed_entity`, `reject_proposed_entity`, `add_relationship`) | `x-api-key` header |
...
- **Hooks** (`.claude/hooks/`): `draft-issue-gate` + `pre-pr-review-gate` (deny), `worktree-gate` (advisory), `worktree-edit-gate` (deny `.js` edits outside a worktree; stands down in cloud sessions), `cloud-issue-gate` (deny ALL repo edits in cloud sessions until an issue number is in `.claude/.cloud-issue-done`), `session-start` (bootstrap Node deps on cloud/remote). Wired + an `rm` deny in `.claude/settings.json`; direct-invocation tests in `hooks/test-gates.sh`. `settings.json` also registers the cloud-gated `devsession-claude` `SessionEnd`/`PreCompact` session-capture hook — it runs only when `CLAUDE_CODE_REMOTE=true` (no-op locally, where a user-level hook handles capture) and no-ops without `LIFECONTEXT_API_KEY`; see `connectors/devsession-claude/README.md`. It also registers the `gh-event-claude` `PostToolUse` hook on the issue/PR create **and `gh pr merge`/MCP-merge** matchers (the create matchers mirror the `draft-issue-gate`/`pre-pr-review-gate` `PreToolUse` gates), which records each created **or merged** issue/PR as an `x-dev-event` memory — a merge is stored under a distinct `<url>#merged` source_id so it never upserts over the "Opened" event (#167). Registered **unguarded** (fires locally and in cloud; harmless double-fire since ingest upserts by source_id and there's no LLM step) and likewise no-ops without `LIFECONTEXT_API_KEY`; see `connectors/gh-event-claude/README.md` (#89, #167). All capture-hook commands use `${CLAUDE_PROJECT_DIR:-$(pwd)}` (like the gate `.sh` scripts) so an unset `CLAUDE_PROJECT_DIR` falls back to the cwd instead of silently no-opping (#167).
