...
3. **`/pre-pr-review`** — run the multi-persona review (or **`/pre-doc-review`** for doc-only PRs). On APPROVE/APPROVE-WITH-NITS, if a governing issue number is on record for the branch (`.claude/.draft-issue-done` / `.claude/.cloud-issue-done` holds a value, or a commit already carries `Closes #`/`Refs #`), open the PR immediately with the generated title/body skeleton (body starts with `Closes #<n>`) — no separate ask. Otherwise report the verdict and wait to be asked. (Gate denies `gh pr create` without an APPROVE marker.) The repo ruleset requires a PR to the default branch (`2.0` today); merge, then sync it.
